...
const DotField = memo(
  ({
    ...
    bulgeStrength = 67,
    glowRadius = 160,
    gradientFrom = "rgba(125,211,252,0.30)",
    gradientTo = "rgba(186,230,253,0.20)",
    glowColor = "#0E0F13",
  }: DotFieldProps) => {
    ...
    const propsRef = useRef<DotFieldEngineProps>({
      dotRadius,
      dotSpacing,
      cursorRadius,
      bulgeStrength,
    ...
    useEffect(() => {
      ...
      function tick() {
        ...
        for (let i = 0; i < len; i++) {
          ...
          context.moveTo(d.sx + rad, d.sy)
          context.arc(d.sx, d.sy, rad, 0, TWO_PI)
        }
      ...
      visibilityObserver.observe(cnv)
